...
const MisRecursos = () => {
  ...
  // Obtener el userData del contexto si está disponible, sino usar el de recursos
  ...
  const handleSubmitRecurso = async (e) => {
    ...
    const datosRecurso = { ...recursoActual }
    ...
    if (modoEdicion) {
      ...
      const resultado = await crearRecurso(datosRecurso, esCategoriaLinks() ? null : archivoFormulario)
      if (!resultado.error) {
        setMostrarModal(false)
        setArchivoFormulario(null)
        setNombreArchivo("")
  ...
  const handleChangeRecurso = (e) => {
    ...
    setRecursoActual((prev) => ({
      ...prev,
      [name]: name === "id_categoria" || name === "id_asignatura" ? Number.parseInt(value) || value : value,
    }))
  }
  ...
  const getNombreAsignatura = (idAsignatura) => {
    // Buscar primero en asignaturas filtradas
    const asignatura = asignaturasFiltradas.find((a) => a.id_asignatura === idAsignatura)
    if (asignatura) return asignatura.nombre_asignatura

    // Si no se encuentra, buscar en todas las asignaturas
    const asignaturaTodas = todasAsignaturas.find((a) => a.id_asignatura === idAsignatura)
    return asignaturaTodas ? asignaturaTodas.nombre_asignatura : "Sin asignatura"
  }
        ...
        <div className="modal-contenido-recursos">
          ...
          <form onSubmit={handleSubmitRecurso}>
            <div className="modal-cuerpo-recursos">
              {modoEdicion && (
                <>
                  ...
                  <div className="campo-formulario-recursos">
                    <label>URL actual del recurso:</label>
                    {recursoActual.URL ? (
                      <div className="url-actual-modal">
                        <input
                          type="text"
                          value={recursoActual.URL}
                          disabled
                          className="input-formulario-recursos disabled url-input"
                        />
                        <a
              ...
              <div className="campo-formulario-recursos">
                ...
                {cargandoAsignaturas && (
                  <small className="cargando-asignaturas-modal">Cargando asignaturas de tu carrera...</small>
                )}
                {userData?.carrera_nombre && (
                  <small className="info-carrera-filtro">
                    Mostrando asignaturas de: <strong>{userData.carrera_nombre}</strong>
                  </small>
                )}
              </div>
                  ...
                  {esCategoriaLinks() ? (
                    <div className="campo-url-manual">
                      ...
                      <div className="info-url">
                        <small>
                          Ingresa la URL completa del recurso web (YouTube, artículos, sitios educativos, etc.)
                        </small>
                      </div>
                    </div>
                  ) : (
                      ...
                      {!archivoFormulario ? (
                        ...
                        <div className="archivo-seleccionado">
                          <span className="nombre-archivo">
                            {recursoActual.id_categoria === 1 ? "🖼️" : recursoActual.id_categoria === 3 ? "🎬" : "📄"}{" "}
                            {nombreArchivo}
                          </span>
                          <button type="button" className="boton-quitar-archivo" onClick={handleQuitarArchivo}>
                            ×
                          </button>
                        </div>
                      )}
              ...
              <div className="campo-formulario-recursos">
                ...
                <div className="campo-usuario-bloqueado">
                  ...
                  <span className="info-usuario-actual">ⓘ El recurso se asociará automáticamente a tu usuario</span>
                </div>
              </div>
            ...
            <div className="modal-pie-recursos">
              <button type="button" className="boton-cancelar-recursos" onClick={() => setMostrarModal(false)}>
                Cancelar
              </button>
              <button
                type="submit"
  ...
  if (!cargando && !recursos.length)
    return (
      <>
        ...
        {/* Modal debe estar disponible incluso cuando no hay recursos */}
        {renderModalRecurso()}
      </>
    )
        ...
        <div className="titulo-recursos-con-boton">
          <div>
            ...
            <p className="mensaje-suspension">
              ⚠ Los recursos con más de <strong>5 reportes</strong> serán <strong>suspendidos</strong>.
            </p>
          </div>
          <button className="boton-nuevo-recurso" onClick={handleNuevoRecurso}>
            + Nuevo Recurso
        ...
        <div className="controles-recursos">
          ...
          <div className="filtros-avanzados">
            <div className="filtro-item">
              ...
              {cargandoAsignaturas && <small className="cargando-asignaturas">Cargando asignaturas...</small>}
            </div>
              ...
              return (
                <tr key={recurso.id_recurso} className={`fila-recurso ${!estaActivo ? "recurso-inactivo" : ""}`}>
                  ...
                  <td className="celda-titulo-recurso">
                    <div className="titulo-recurso">{recurso.titulo}</div>
                  </td>
                  ...
                  <td className="celda-url-recurso">
                    {recurso.URL ? (
                      <div className="url-info">
                        <button onClick={() => copiarURL(recurso.URL)} className="boton-copiar-url" title="Copiar URL">
                          📋
                        </button>
                        <span
                          className="texto-url"
                  ...
                  <td className="celda-asignatura-recurso">{getNombreAsignatura(recurso.id_asignatura)}</td>
                  <td className="celda-categoria-recurso">
                    <div className="badge-categoria">{getNombreCategoria(recurso.id_categoria)}</div>
                  </td>
                  ...
                  <td className="celda-reportes-recurso">
                    {recurso.contador_reportes > 0 ? (
                      <div
                        className={`badge-reportes ${
                          recurso.contador_reportes >= 5
                            ? "reportes-criticos"
                            : recurso.contador_reportes >= 3
                              ? "reportes-alerta"
                              : "reportes-normal"
                        }`}
                      >
                        {recurso.contador_reportes}
                      </div>
                    ) : (
                      <span className="sin-reportes">0</span>
                    )}
                  </td>
                  <td className="celda-acciones-recurso">
                    <div className="acciones-recurso">
                      <button
                        className="boton-accion-recurso boton-editar"
                      ...
                      <button
                        className="boton-accion-recurso boton-eliminar"
                      ...
                      <button
                        className={`boton-accion-recurso ${estaActivo ? "boton-desactivar" : "boton-activar"}`}
                        onClick={() => handleToggleEstado(recurso)}
                        title={estaActivo ? "Desactivar recurso" : "Activar recurso"}
                      >
                        {estaActivo ? "🔒" : "🔓"}
                      </button>
                    </div>
        ...
        <div className="modal-fondo-recursos">
          <div className="modal-contenido-recursos modal-confirmacion">
            <div className="modal-cabecera-recursos">
              ...
              <button className="modal-cerrar-recursos" onClick={() => setMostrarConfirmacionEliminar(false)}>
                ×
              </button>
            </div>
            ...
            <div className="modal-cuerpo-recursos">
              ...
              <div className="info-eliminar-detalle">
                <p>
                  <strong>Asignatura:</strong> {getNombreAsignatura(recursoAEliminar?.id_asignatura)}
                </p>
                <p>
                  <strong>Categoría:</strong> {getNombreCategoria(recursoAEliminar?.id_categoria)}
                </p>
                <p>
                  <strong>Tipo:</strong> {getTipoArchivo(recursoAEliminar?.URL)}
                </p>
                <p>
                  <strong>Fecha:</strong> {new Date(recursoAEliminar?.fecha_subida).toLocaleDateString("es-ES")}
                </p>
              </div>
              ...
              {recursoAEliminar?.URL && (
                <div className="url-eliminar">
                  <p>
                    <strong>URL:</strong> <span className="texto-url">{recursoAEliminar.URL.substring(0, 50)}...</span>
                  </p>
                  <button className="boton-copiar-pequeno" onClick={() => copiarURL(recursoAEliminar.URL)}>
                    📋 Copiar URL
                  </button>
                </div>
              )}
            ...
            <div className="modal-pie-recursos">
              ...
              <button className="boton-eliminar-confirmar" onClick={confirmarEliminarRecurso} disabled={cargando}>
      ...
      {recursos.length > 0 && (
        <div className="paginador-recursos">
          <div className="info-paginacion-recursos">
            Mostrando {indicePrimerElemento + 1} - {Math.min(indiceUltimoElemento, recursos.length)} de{" "}
